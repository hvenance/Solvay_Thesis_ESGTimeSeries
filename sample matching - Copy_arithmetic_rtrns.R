...
colnames(ESG)
country_ESG <- ESG %>%   
  dplyr::count(`Country of Exchange`)

country_nonESG <- nonESG %>% 
  dplyr::count(`Country of Exchange`)

country <- merge(country_ESG, country_nonESG, by="Country of Exchange", all.x = T, all.y = T)

names(country)[2] <- "Leaders"
names(country)[3] <- "Laggards"
leaders_percent <- country$Leaders/ sum(country$Leaders, na.rm=T) * 100
laggards_percent <- country$Laggards/ sum(country$Laggards, na.rm=T) * 100
...
#only take data until 31st May 2020
ESG_timeserie <- ESG_timeserie[ESG_timeserie$Instrument<="2020-12-31",]
ESG_timeserie <- ESG_timeserie[ESG_timeserie$Instrument>="2020-01-01",]
...
n_row = length(ESG_timeserie[,1])
date <- tech_ESG_timeserie$date
`SAP SE`<- 100*((tech_ESG_timeserie[2:n_row,2]-tech_ESG_timeserie[1:(n_row-1),2])/tech_ESG_timeserie[1:(n_row-1),2])
tech_ESG_timeserie <- rbind(NA,`SAP SE`)
tech_ESG_timeserie$date <- date
tech_ESG_timeserie<- tech_ESG_timeserie%>%
  relocate(date, .before=`SAP SE`)


colnames(nonESG_timeserie) <- nonESG_timeserie[1,]
nonESG_timeserie <- nonESG_timeserie[-c(1:2),]
#only take data until 31st May 2020
nonESG_timeserie <- nonESG_timeserie[nonESG_timeserie$Instrument<="2020-12-31",]
nonESG_timeserie <- nonESG_timeserie[nonESG_timeserie$Instrument>="2020-01-01",]
...
date <- tech_nonESG_timeserie$date
"All for One Group SE"<- data.frame(100*((tech_nonESG_timeserie[2:n_row,2]-tech_nonESG_timeserie[1:(n_row-1),2])/tech_nonESG_timeserie[1:(n_row-1),2]))
`All for One Group SE` <- rbind(NA,`All for One Group SE`)
colnames(`All for One Group SE`) <- "All for One Group SE"
`All for One Group SE`$date <- date
`All for One Group SE` <- `All for One Group SE`%>%
  relocate(date, .before=`All for One Group SE`)

`SUESS MicroTec SE`<- data.frame(100*((tech_nonESG_timeserie[2:n_row,3]-tech_nonESG_timeserie[1:(n_row-1),3])/tech_nonESG_timeserie[1:(n_row-1),3]))
`SUESS MicroTec SE` <- rbind(NA,`SUESS MicroTec SE`)
colnames(`SUESS MicroTec SE`) <- "SUESS MicroTec SE"
`SUESS MicroTec SE`$date <- date
`SUESS MicroTec SE`<- `SUESS MicroTec SE`%>%
  relocate(date, .before=`SUESS MicroTec SE`)

`PSI Software AG`<- data.frame(100*((tech_nonESG_timeserie[2:n_row,4]-tech_nonESG_timeserie[1:(n_row-1),4])/tech_nonESG_timeserie[1:(n_row-1),4]))
`PSI Software AG` <- rbind(NA,`PSI Software AG`)
colnames(`PSI Software AG`) <- "PSI Software AG"
`PSI Software AG`$date <- date
`PSI Software AG`<- `PSI Software AG`%>%
  relocate(date, .before=`PSI Software AG`)

tech_nonESG_timeserie1 <- merge(`All for One Group SE`,`SUESS MicroTec SE`, by= "date")
tech_nonESG_timeserie1 <- merge(tech_nonESG_timeserie1, `PSI Software AG`, by="date")

#merge the 2dfs
tech_merged <- merge(tech_ESG_timeserie, tech_nonESG_timeserie1, by="date")
...
#create rtrns based on price
colnames(health_ESG_timeserie)
date <- health_ESG_timeserie$date
"Roche Holding AG"<- data.frame(100*((health_ESG_timeserie[2:n_row,2]-health_ESG_timeserie[1:(n_row-1),2])/health_ESG_timeserie[1:(n_row-1),2]))
`Roche Holding AG` <- rbind(NA,`Roche Holding AG`)
colnames(`Roche Holding AG`) <- "Roche Holding AG"
`Roche Holding AG`$date <- date
`Roche Holding AG` <- `Roche Holding AG`%>%
  relocate(date, .before=`Roche Holding AG`)

"Lonza Group AG"<- data.frame(100*((health_ESG_timeserie[2:n_row,2]-health_ESG_timeserie[1:(n_row-1),2])/health_ESG_timeserie[1:(n_row-1),2]))
`Lonza Group AG` <- rbind(NA,`Lonza Group AG`)
colnames(`Lonza Group AG`) <- "Lonza Group AG"
`Lonza Group AG`$date <- date
`Lonza Group AG` <- `Lonza Group AG`%>%
  relocate(date, .before=`Lonza Group AG`)

health_ESG_timeserie1 <- merge(`Roche Holding AG`, `Lonza Group AG`, by = "date")
...
date <- health_nonESG_timeserie$date
"Coltene Holding AG"<- data.frame(100*((health_nonESG_timeserie[2:n_row,2]-health_nonESG_timeserie[1:(n_row-1),2])/health_nonESG_timeserie[1:(n_row-1),2]))
`Coltene Holding AG` <- rbind(NA,`Coltene Holding AG`)
colnames(`Coltene Holding AG`) <- "Coltene Holding AG"
`Coltene Holding AG`$date <- date
`Coltene Holding AG` <- `Coltene Holding AG`%>%
  relocate(date, .before=`Coltene Holding AG`)

`Cosmo Pharmaceuticals NV`<- data.frame(100*((health_nonESG_timeserie[2:n_row,3]-health_nonESG_timeserie[1:(n_row-1),3])/health_nonESG_timeserie[1:(n_row-1),3]))
`Cosmo Pharmaceuticals NV` <- rbind(NA,`Cosmo Pharmaceuticals NV`)
colnames(`Cosmo Pharmaceuticals NV`) <- "Cosmo Pharmaceuticals NV"
`Cosmo Pharmaceuticals NV`$date <- date
`Cosmo Pharmaceuticals NV`<- `Cosmo Pharmaceuticals NV`%>%
  relocate(date, .before=`Cosmo Pharmaceuticals NV`)

health_nonESG_timeserie1 <- merge(`Coltene Holding AG`, `Cosmo Pharmaceuticals NV`, by= "date")

#merge the 2dfs
health_merged <- merge(health_ESG_timeserie1, health_nonESG_timeserie1, by="date")
...
#create rtrns based on price
colnames(industrials_ESG_timeserie)
date <- industrials_ESG_timeserie$date
"Mondi PLC"<- data.frame(100*((industrials_ESG_timeserie[2:n_row,2]-industrials_ESG_timeserie[1:(n_row-1),2])/industrials_ESG_timeserie[1:(n_row-1),2]))
`Mondi PLC` <- rbind(NA,`Mondi PLC`)
colnames(`Mondi PLC`) <- "Mondi PLC"
`Mondi PLC`$date <- date
`Mondi PLC` <- `Mondi PLC`%>%
  relocate(date, .before=`Mondi PLC`)

"Alfa Laval AB"<- data.frame(100*((industrials_ESG_timeserie[2:n_row,2]-industrials_ESG_timeserie[1:(n_row-1),2])/industrials_ESG_timeserie[1:(n_row-1),2]))
`Alfa Laval AB` <- rbind(NA,`Alfa Laval AB`)
colnames(`Alfa Laval AB`) <- "Alfa Laval AB"
`Alfa Laval AB`$date <- date
`Alfa Laval AB` <- `Alfa Laval AB`%>%
  relocate(date, .before=`Alfa Laval AB`)

industrials_ESG_timeserie1 <- merge(`Mondi PLC`, `Alfa Laval AB`, by = "date")
...
date <- industrials_nonESG_timeserie$date
"Judges Scientific PLC"<- data.frame(100*((industrials_nonESG_timeserie[2:n_row,2]-industrials_nonESG_timeserie[1:(n_row-1),2])/industrials_nonESG_timeserie[1:(n_row-1),2]))
`Judges Scientific PLC` <- rbind(NA,`Judges Scientific PLC`)
colnames(`Judges Scientific PLC`) <- "Judges Scientific PLC"
`Judges Scientific PLC`$date <- date
`Judges Scientific PLC` <- `Judges Scientific PLC`%>%
  relocate(date, .before=`Judges Scientific PLC`)

`Marlowe PLC`<- data.frame(100*((industrials_nonESG_timeserie[2:n_row,3]-industrials_nonESG_timeserie[1:(n_row-1),3])/industrials_nonESG_timeserie[1:(n_row-1),3]))
`Marlowe PLC` <- rbind(NA,`Marlowe PLC`)
colnames(`Marlowe PLC`) <- "Marlowe PLC"
`Marlowe PLC`$date <- date
`Marlowe PLC`<- `Marlowe PLC`%>%
  relocate(date, .before=`Marlowe PLC`)

"Jetpak Top Holding AB (publ)"<- data.frame(100*((industrials_nonESG_timeserie[2:n_row,4]-industrials_nonESG_timeserie[1:(n_row-1),4])/industrials_nonESG_timeserie[1:(n_row-1),4]))
`Jetpak Top Holding AB (publ)` <- rbind(NA,`Jetpak Top Holding AB (publ)`)
colnames(`Jetpak Top Holding AB (publ)`) <- "Jetpak Top Holding AB (publ)"
`Jetpak Top Holding AB (publ)`$date <- date
`Jetpak Top Holding AB (publ)` <- `Jetpak Top Holding AB (publ)`%>%
  relocate(date, .before=`Jetpak Top Holding AB (publ)`)

industrials_nonESG_timeserie1 <- merge(`Judges Scientific PLC`, `Marlowe PLC`, by= "date")
industrials_nonESG_timeserie1 <- merge(industrials_nonESG_timeserie1, `Jetpak Top Holding AB (publ)`, by= "date")

#merge the 2dfs
industrials_merged <- merge(industrials_ESG_timeserie1, industrials_nonESG_timeserie1, by="date")
...
#create rtrns based on price
colnames(financials_ESG_timeserie)
date <- financials_ESG_timeserie$date
"Assicurazioni Generali SpA"<- data.frame(100*((financials_ESG_timeserie[2:n_row,2]-financials_ESG_timeserie[1:(n_row-1),2])/financials_ESG_timeserie[1:(n_row-1),2]))
`Assicurazioni Generali SpA` <- rbind(NA,`Assicurazioni Generali SpA`)
colnames(`Assicurazioni Generali SpA`) <- "Assicurazioni Generali SpA"
`Assicurazioni Generali SpA`$date <- date
`Assicurazioni Generali SpA` <- `Assicurazioni Generali SpA`%>%
  relocate(date, .before=`Assicurazioni Generali SpA`)

"Allianz SE"<- data.frame(100*((financials_ESG_timeserie[2:n_row,3]-financials_ESG_timeserie[1:(n_row-1),3])/financials_ESG_timeserie[1:(n_row-1),3]))
`Allianz SE` <- rbind(NA,`Allianz SE`)
colnames(`Allianz SE`) <- "Allianz SE"
`Allianz SE`$date <- date
`Allianz SE` <- `Allianz SE`%>%
  relocate(date, .before=`Allianz SE`)

"Zurich Insurance Group AG"<- data.frame(100*((financials_ESG_timeserie[2:n_row,4]-financials_ESG_timeserie[1:(n_row-1),4])/financials_ESG_timeserie[1:(n_row-1),4]))
`Zurich Insurance Group AG` <- rbind(NA,`Zurich Insurance Group AG`)
colnames(`Zurich Insurance Group AG`) <- "Zurich Insurance Group AG"
`Zurich Insurance Group AG`$date <- date
`Zurich Insurance Group AG` <- `Zurich Insurance Group AG`%>%
  relocate(date, .before=`Zurich Insurance Group AG`)

financials_ESG_timeserie1 <- merge(`Assicurazioni Generali SpA`, `Allianz SE`, by = "date")
financials_ESG_timeserie1 <- merge(financials_ESG_timeserie1, `Zurich Insurance Group AG`, by = "date")
...
date <- financials_nonESG_timeserie$date
"Tamburi Investment Partners SpA"<- data.frame(100*((financials_nonESG_timeserie[2:n_row,2]-financials_nonESG_timeserie[1:(n_row-1),2])/financials_nonESG_timeserie[1:(n_row-1),2]))
`Tamburi Investment Partners SpA` <- rbind(NA,`Tamburi Investment Partners SpA`)
colnames(`Tamburi Investment Partners SpA`) <- "Tamburi Investment Partners SpA"
`Tamburi Investment Partners SpA`$date <- date
`Tamburi Investment Partners SpA` <- `Tamburi Investment Partners SpA`%>%
  relocate(date, .before=`Tamburi Investment Partners SpA`)

`flatexDEGIRO AG`<- data.frame(100*((financials_nonESG_timeserie[2:n_row,3]-financials_nonESG_timeserie[1:(n_row-1),3])/financials_nonESG_timeserie[1:(n_row-1),3]))
`flatexDEGIRO AG` <- rbind(NA,`flatexDEGIRO AG`)
colnames(`flatexDEGIRO AG`) <- "flatexDEGIRO AG"
`flatexDEGIRO AG`$date <- date
`flatexDEGIRO AG`<- `flatexDEGIRO AG`%>%
  relocate(date, .before=`flatexDEGIRO AG`)

"Swissquote Group Holding SA"<- data.frame(100*((financials_nonESG_timeserie[2:n_row,4]-financials_nonESG_timeserie[1:(n_row-1),4])/financials_nonESG_timeserie[1:(n_row-1),4]))
`Swissquote Group Holding SA` <- rbind(NA,`Swissquote Group Holding SA`)
colnames(`Swissquote Group Holding SA`) <- "Swissquote Group Holding SA"
`Swissquote Group Holding SA`$date <- date
`Swissquote Group Holding SA` <- `Swissquote Group Holding SA`%>%
  relocate(date, .before=`Swissquote Group Holding SA`)

"Private Equity Holding AG"<- data.frame(100*((financials_nonESG_timeserie[2:n_row,5]-financials_nonESG_timeserie[1:(n_row-1),5])/financials_nonESG_timeserie[1:(n_row-1),5]))
`Private Equity Holding AG` <- rbind(NA,`Private Equity Holding AG`)
colnames(`Private Equity Holding AG`) <- "Private Equity Holding AG"
`Private Equity Holding AG`$date <- date
`Private Equity Holding AG` <- `Private Equity Holding AG`%>%
  relocate(date, .before=`Private Equity Holding AG`)

financials_nonESG_timeserie1 <- merge(`Tamburi Investment Partners SpA`, `flatexDEGIRO AG`, by= "date")
financials_nonESG_timeserie1 <- merge(financials_nonESG_timeserie1, `Swissquote Group Holding SA`, by= "date")
financials_nonESG_timeserie1 <- merge(financials_nonESG_timeserie1, `Private Equity Holding AG`, by= "date")

#merge the 2dfs
financials_merged <- merge(financials_ESG_timeserie1, financials_nonESG_timeserie1, by="date")
...
#create rtrns based on price
colnames(utilities_ESG_timeserie)
date <- utilities_ESG_timeserie$date
"Iberdrola SA"<- data.frame(100*((utilities_ESG_timeserie[2:n_row,2]-utilities_ESG_timeserie[1:(n_row-1),2])/utilities_ESG_timeserie[1:(n_row-1),2]))
`Iberdrola SA` <- rbind(NA,`Iberdrola SA`)
colnames(`Iberdrola SA`) <- "Iberdrola SA"
`Iberdrola SA`$date <- date
`Iberdrola SA` <- `Iberdrola SA`%>%
  relocate(date, .before=`Iberdrola SA`)

"Naturgy Energy Group SA"<- data.frame(100*((utilities_ESG_timeserie[2:n_row,3]-utilities_ESG_timeserie[1:(n_row-1),3])/utilities_ESG_timeserie[1:(n_row-1),3]))
`Naturgy Energy Group SA` <- rbind(NA,`Naturgy Energy Group SA`)
colnames(`Naturgy Energy Group SA`) <- "Naturgy Energy Group SA"
`Naturgy Energy Group SA`$date <- date
`Naturgy Energy Group SA` <- `Naturgy Energy Group SA`%>%
  relocate(date, .before=`Naturgy Energy Group SA`)

"Red Electrica Corporacion SA"<- data.frame(100*((utilities_ESG_timeserie[2:n_row,4]-utilities_ESG_timeserie[1:(n_row-1),4])/utilities_ESG_timeserie[1:(n_row-1),4]))
`Red Electrica Corporacion SA` <- rbind(NA,`Red Electrica Corporacion SA`)
colnames(`Red Electrica Corporacion SA`) <- "Red Electrica Corporacion SA"
`Red Electrica Corporacion SA`$date <- date
`Red Electrica Corporacion SA` <- `Red Electrica Corporacion SA`%>%
  relocate(date, .before=`Red Electrica Corporacion SA`)

utilities_ESG_timeserie1 <- merge(`Iberdrola SA`, `Naturgy Energy Group SA`, by = "date")
utilities_ESG_timeserie1 <- merge(utilities_ESG_timeserie1, `Red Electrica Corporacion SA`, by = "date")
...
#create rtrns based on price
`Solaria Energia y Medio Ambiente SA`<- data.frame(100*((utilities_nonESG_timeserie[2:n_row,2]-utilities_nonESG_timeserie[1:(n_row-1),2])/utilities_nonESG_timeserie[1:(n_row-1),2]))
utilities_nonESG_timeserie <- rbind(NA,`Solaria Energia y Medio Ambiente SA`)
colnames(utilities_nonESG_timeserie) <- "Solaria Energia y Medio Ambiente SA"
utilities_nonESG_timeserie$date <- date
utilities_nonESG_timeserie<- utilities_nonESG_timeserie%>%
  relocate(date, .before=`Solaria Energia y Medio Ambiente SA`)

#merge the 2dfs
utilities_merged <- merge(utilities_ESG_timeserie1, utilities_nonESG_timeserie, by="date")
...
#create rtrns based on price
colnames(consumer_ESG_timeserie)
date <- consumer_ESG_timeserie$date
"Pearson PLC"<- data.frame(100*((consumer_ESG_timeserie[2:n_row,2]-consumer_ESG_timeserie[1:(n_row-1),2])/consumer_ESG_timeserie[1:(n_row-1),2]))
`Pearson PLC` <- rbind(NA,`Pearson PLC`)
colnames(`Pearson PLC`) <- "Pearson PLC"
`Pearson PLC`$date <- date
`Pearson PLC` <- `Pearson PLC`%>%
  relocate(date, .before=`Pearson PLC`)

`Whitbread PLC`<- data.frame(100*((consumer_ESG_timeserie[2:n_row,3]-consumer_ESG_timeserie[1:(n_row-1),3])/consumer_ESG_timeserie[1:(n_row-1),3]))
`Whitbread PLC` <- rbind(NA,`Whitbread PLC`)
colnames(`Whitbread PLC`) <- "Whitbread PLC"
`Whitbread PLC`$date <- date
`Whitbread PLC`<- `Whitbread PLC`%>%
  relocate(date, .before=`Whitbread PLC`)

"Elior Group SA"<- data.frame(100*((consumer_ESG_timeserie[2:n_row,4]-consumer_ESG_timeserie[1:(n_row-1),4])/consumer_ESG_timeserie[1:(n_row-1),4]))
`Elior Group SA` <- rbind(NA,`Elior Group SA`)
colnames(`Elior Group SA`) <- "Elior Group SA"
`Elior Group SA`$date <- date
`Elior Group SA` <- `Elior Group SA`%>%
  relocate(date, .before=`Elior Group SA`)

"LOreal SA"<- data.frame(100*((consumer_ESG_timeserie[2:n_row,5]-consumer_ESG_timeserie[1:(n_row-1),5])/consumer_ESG_timeserie[1:(n_row-1),5]))
`LOreal SA` <- rbind(NA,`LOreal SA`)
colnames(`LOreal SA`) <- "LOreal SA"
`LOreal SA`$date <- date
`LOreal SA` <- `LOreal SA`%>%
  relocate(date, .before=`LOreal SA`)

consumer_ESG_timeserie1 <- merge(`Pearson PLC`, `Whitbread PLC`, by= "date")
consumer_ESG_timeserie1 <- merge(consumer_ESG_timeserie1, `Elior Group SA`, by= "date")
consumer_ESG_timeserie1 <- merge(consumer_ESG_timeserie1, `LOreal SA`, by= "date")
...
date <- consumer_nonESG_timeserie$date
"Trainline PLC"<- data.frame(100*((consumer_nonESG_timeserie[2:n_row,2]-consumer_nonESG_timeserie[1:(n_row-1),2])/consumer_nonESG_timeserie[1:(n_row-1),2]))
`Trainline PLC` <- rbind(NA,`Trainline PLC`)
colnames(`Trainline PLC`) <- "Trainline PLC"
`Trainline PLC`$date <- date
`Trainline PLC` <- `Trainline PLC`%>%
  relocate(date, .before=`Trainline PLC`)

`CVS Group PLC`<- data.frame(100*((consumer_nonESG_timeserie[2:n_row,3]-consumer_nonESG_timeserie[1:(n_row-1),3])/consumer_nonESG_timeserie[1:(n_row-1),3]))
`CVS Group PLC` <- rbind(NA,`CVS Group PLC`)
colnames(`CVS Group PLC`) <- "CVS Group PLC"
`CVS Group PLC`$date <- date
`CVS Group PLC`<- `CVS Group PLC`%>%
  relocate(date, .before=`CVS Group PLC`)

"Interparfums SA"<- data.frame(100*((consumer_nonESG_timeserie[2:n_row,4]-consumer_nonESG_timeserie[1:(n_row-1),4])/consumer_nonESG_timeserie[1:(n_row-1),4]))
`Interparfums SA` <- rbind(NA,`Interparfums SA`)
colnames(`Interparfums SA`) <- "Interparfums SA"
`Interparfums SA`$date <- date
`Interparfums SA` <- `Interparfums SA`%>%
  relocate(date, .before=`Interparfums SA`)

"Trigano SA"<- data.frame(100*((consumer_nonESG_timeserie[2:n_row,5]-consumer_nonESG_timeserie[1:(n_row-1),5])/consumer_nonESG_timeserie[1:(n_row-1),5]))
`Trigano SA` <- rbind(NA,`Trigano SA`)
colnames(`Trigano SA`) <- "Trigano SA"
`Trigano SA`$date <- date
`Trigano SA` <- `Trigano SA`%>%
  relocate(date, .before=`Trigano SA`)

consumer_nonESG_timeserie1 <- merge(`Trainline PLC`, `CVS Group PLC`, by= "date")
consumer_nonESG_timeserie1 <- merge(consumer_nonESG_timeserie1, `Interparfums SA`, by= "date")
consumer_nonESG_timeserie1 <- merge(consumer_nonESG_timeserie1, `Trigano SA`, by= "date")

#merge the 2dfs
consumer_merged <- merge(consumer_ESG_timeserie1, consumer_nonESG_timeserie1, by="date")
